...
class Tier(object):
    ...
    def make_point_list(self):
        """
        @return: a list of point objects in this tier, 1-indexed
        """
        point_list = []
        for i in range(1,len(self.simple_transcript)+1):
            point_list.append(Point(i,self.simple_transcript[i-1][0],self.simple_transcript[i-1][1]))
        return point_list
...
class Point(object):
    def __init__(self,pointNum,time,mark):
        self.pointNum = pointNum
        self.time = time
        self.mark = mark

    def __str__(self):
        return '(Point '+str(self.pointNum)+' at time '+str(self.time)+' with mark '+str(self.mark)+')\n'
